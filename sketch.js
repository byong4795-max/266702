...
let handpose;
let predictions = [];
let gameState = 'START'; // START, L1, L2, L3, FAIL, SUCCESS
let score = 0;
let levelStep = 0;
let timer = 0;
...
function drawQRWaitScreen() {
  ...
  // QRcode 會由外部 logic 產生在畫面上
  if (predictions.length > 0 || (video && video.loadedmetadata)) {
    // 偵測到有影像或連線，準備進入介紹
    pendingLevel = "L1";
    gameState = "INTRO";
  }
...
function nextL2Question() {
  ...
  let diffCol = color(red(baseCol) + (random() > 0.5 ? 20 : -20), green(baseCol), blue(baseCol));
...
function drawFailScreen() {
  ...
  if (failMsg.includes("醫鬧")) {
    // 簡易醫鬧動畫：生氣跳動的圓形
    drawHuman(width/2 + sin(frameCount * 0.5) * 50, height/2 + 100, 1.0, [0, 200, 0]); // 換個綠色衣服
    circle(width/2 + sin(frameCount * 0.5) * 50, height/2 + 100 + cos(frameCount * 0.8) * 20, 50);
    text("💢", width/2 + sin(frameCount * 0.5) * 50, height/2 + 100);
  }
